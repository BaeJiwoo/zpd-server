...
try:
    ...
    first.expect("(2/2)")
    first.command("/game 1")
    first.expect("Game response: 1")
    first.expect(f"Game event 1: {ids[0]} 1")
    second.expect(f"Game event 1: {ids[0]} 1")
    second.command("/game 2")
    second.expect("Game response: 0")
    first.command("/move nope 0 0")
    first.expect("Invalid position")
    first.command("/move 10 -2 3")
    first.expect("Position accepted for next tick")
    first.expect(f"Player {ids[0]} position: 10 -2 3")
    second.expect(f"Player {ids[0]} position: 10 -2 3")
    second.command("/positions")
    second.expect("Position tick:")
    second.expect(f"Player {ids[0]} position: 10 -2 3")
    first.command("안녕하세요 👋")

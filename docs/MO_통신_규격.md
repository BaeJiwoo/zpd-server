...
`common/proto/position.proto`에 위치 요청(8), 응답(136), 방 전체 위치 알림(196)을 추가했습니다. 기본 20Hz이며 기존 클라이언트도 새 알림 처리 구현이 필요합니다. [상세 규격과 테스트](틱_기반_위치_동기화.md)를 참고하세요.

## 게임 로직 명령 확장

`common/proto/game.proto`는 GameCommandRequest(9), GameCommandResponse(137), GameEvent(197)를 정의합니다. 사용자 정의 payload는 최대 4000바이트이며, 명령은 현재 방 ID와 0이 아닌 command가 필요합니다. 알림은 0이 아닌 event와 requestId=0을 사용합니다. [게임 로직 개발 가이드](게임_로직_개발_가이드.md)에 예제와 처리 경계를 설명합니다.
